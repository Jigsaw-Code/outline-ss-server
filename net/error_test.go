...
func TestIsConnectionError(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "WrongStatus",
			err:    &ConnectionError{Message: "connection error"},
			status: "STATUS_FOO",
			want:   false,
		},
		{
		...
		{
			name:   "CorrectStatus",
			err:    &ConnectionError{Status: "STATUS_FOO", Message: "connection error"},
			status: "STATUS_FOO",
			want:   true,
		},
		{
			name:   "DifferentStatus",
			err:    &ConnectionError{Status: "STATUS_BAR", Message: "connection error"},
			status: "STATUS_FOO",
			want:   false,
		},
		{
			name:   "EmptyStatus",
			err:    &ConnectionError{Status: "", Message: "connection error"},
			status: "STATUS_FOO",
			want:   false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.Equal(t, tt.want, IsConnectionError(tt.err, tt.status))
		})
	}
}
