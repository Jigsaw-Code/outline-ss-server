...
func (s *SSServer) startPort(portNum int) error {
	...
	streamAuthFunc := service.NewShadowsocksStreamAuthenticator(port.cipherList, &s.replayCache, s.m)
	// TODO: Register initial data metrics at zero.
	tcpHandler := service.NewTCPHandler(portNum, streamAuthFunc, s.m, tcpReadTimeout)
	packetAuthFunc := service.NewShadowsocksPacketAuthenticator(port.cipherList, s.m)
	packetHandler := service.NewPacketHandler(s.natTimeout, packetAuthFunc, s.m)
